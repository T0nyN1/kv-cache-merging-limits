...
class OTKVCache(BaseCompressCache):
    def __init__(self, **kwargs):
        ...
        # Optimal transport compression is inherently per-head: importance
        # scores and the transport plan are computed independently per KV head.
        if not kwargs.get("per_head", False):
            import warnings
            warnings.warn(
                f"OT-KV (v2) only implements per-head compression; per_head=False was "
